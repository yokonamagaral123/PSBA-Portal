...
const AdminAttendance = () => {
  ...
  useEffect(() => {
    const fetchAttendance = async () => {
      ...
      if (data.success) {
        ...
        const todayRow = data.records.find(
          (row) => row.date === dateStr && row.employeeID === employeeId
        );
        if (todayRow && todayRow.timeIn && !todayRow.timeOut) {
          setHasLoggedIn(true);
        } else {
  ...
  const handleLogTimeOut = async () => {
    ...
    if (schedStart && schedEnd) {
      ...
      const schStartMinutes = parseInt(schStartH) * 60 + parseInt(schStartM);
      const schEndMinutes = parseInt(schEndH) * 60 + parseInt(schEndM);
      let tag = "";
      if (outMinutes > schEndMinutes) {
        tag = "OVERTIME";
      } else if (outMinutes < schEndMinutes && outMinutes > schStartMinutes) {
        tag = "UNDERTIME";
      }
  ...
  return (
    <>
      ...
      <div className="admin-attendance-filter-row">
        <div className="admin-attendance-log-btn-container">
          <button
            className="admin-attendance-log-btn"
        ...
        <input
          className="admin-attendance-input"
          type="date"
          value={startDate}
        ...
        />
        <button
          className="admin-attendance-reset-btn"
          title="Reset filters"
          onClick={() => {
            setStartDate("");
            setEndDate("");
        ...
        <div
          className="admin-attendance-pagination-bar"
          style={{ marginLeft: "auto" }}
        >
          <button
            onClick={() => goToPage(Math.max(1, currentPage - 10))}
            disabled={currentPage === 1}
            className="admin-attendance-page-btn"
          >
            {"<<"}
          </button>
          <button
          ...
          {getPageNumbers().map((page) => (
            <button
              ...
              className={`admin-attendance-page-btn${
                page === currentPage ? " admin-attendance-page-current" : ""
              }`}
      ...
      {showClockModal && (
        <div className="admin-attendance-modal-overlay">
          <div className="admin-attendance-modal">
            <button
              className="admin-attendance-modal-close"
              onClick={() => setShowClockModal(false)}
            >
              &times;
            </button>
            <div className="admin-attendance-clock-day">
              {clock
                .toLocaleDateString(undefined, { weekday: "long" })
                .toUpperCase()}
            </div>
            <div className="admin-attendance-clock-time">
              {clock.toLocaleTimeString([], { hour12: false })}
            </div>
            <div className="admin-attendance-clock-date">
            ...
            <div className="admin-attendance-clock-btn-row">
              <button
                className="admin-attendance-clock-action-btn"
