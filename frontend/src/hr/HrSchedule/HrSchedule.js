  ...
  useEffect(() => {
    const fetchSchedule = async () => {
      try {
        ...
        setSchedule((data.employee && data.employee.schedule) || {});
      } catch (err) {
        setSchedule({});
      }
    };
  ...
  return (
    <>
          ...
          {weeks.map((week, i) => (
            <div className="hr-calendar-row" key={i}>
              {week.map((date, j) => {
                ...
                return (
                  <div
                    className="hr-calendar-cell hr-calendar-date"
                    key={j}
                    style={{
                      position: 'relative',
                  ...
                  >
                    {date && (
                      <div style={{ position: 'relative', display: 'inline-block' }}>
                        <span className="hr-calendar-date-number">{date}</span>
                        {date === today.getDate() &&
                          currentMonth === today.getMonth() &&
                          currentYear === today.getFullYear() && (
                            <span className="hr-today-dot" style={{
                              position: 'absolute',
                              left: '450%',
      ...
      {hoveredCell && hoveredCell.data && (
        <div
          className="hr-calendar-modal-tooltip"
          style={{
            position: 'fixed',
